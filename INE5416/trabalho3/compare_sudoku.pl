...
validSymbolsOfTable(Symbols, Tab) :-
    length(Tab, L), validSymbolsOfTableAtIndexes(Symbols, Tab, 0, 0, L).
...
validSymbolsOfTableAtIndexes(_, _, _, Len, Len) :- !. % Termina procura
validSymbolsOfTableAtIndexes(Symbols, Tab, Len, Y, Len) :- YM1 is Y+1, validSymbolsOfTableAtIndexes(Symbols, Tab, 0, YM1, Len). % Passa para próxima linha
validSymbolsOfTableAtIndexes(Symbols, Tab, X, Y, Len) :-
    ...
    length(Tab, Len),
    % Process next 
    validSymbolsOfTableAtIndexes(Symbols, Tab, XM1, Y, Len),
    % Get rows from the table to get adjacent values
    getRow(Tab, Len, Ym1, RowUp),
    getRow(Tab, Len, YM1, RowDown),
    nth0(Y, Tab, Row),
